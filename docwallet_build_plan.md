...
- **Password on**: Even with the phone unlocked, an attacker cannot read app
  data without the DocWallet password. The master key is only held in the app
...
Backup creates a single encrypted `.docwallet-backup` file that can be migrated
...
[AES-256-GCM encrypt the entire bundle with a random backup IV]
        │
        ▼
[Write to .docwallet-backup file at chosen SAF location]
```

### Import flow

```
Settings → "Import backup" → SAF file picker (.docwallet-backup)
        │
        ▼
...
│   ├── src/main/
│   │   ├── java/com/docwallet/
│   │   │   ├── DocWalletApplication.kt
│   │   │   ├── MainActivity.kt
│   │   │   │
│   │   │   ├── data/
│   │   │   │   ├── db/
│   │   │   │   │   ├── DocWalletDatabase.kt
│   │   │   │   │   ├── DocumentDao.kt
│   │   │   │   │   ├── TagDao.kt
│   │   │   │   │   ├── CollectionDao.kt
│   │   │   │   │   └── Converters.kt
...
```
app/src/
├── test/java/com/docwallet/                    ← Unit tests (JVM + Robolectric)
│   ├── encryption/
...
│
└── androidTest/java/com/docwallet/             ← Instrumented tests (device/emulator)
    └── ui/
...
- Basic `DocWalletApplication.kt` and `MainActivity.kt` with Compose Navigation scaffold
...
- [ ] **Document scanning**: CameraX + ML Kit document detection → scan paper docs directly into DocWallet
...
- [ ] **OPDS catalog**: Browse public ebook catalogs from within DocWallet
...
3. **PDF Wallet was built by one person** — DocWallet is strictly additive (more formats, encryption, FTS search)
