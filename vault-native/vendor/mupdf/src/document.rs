...
impl Document {
    ...
    /// Number of chapters in the document. Documents without chapters (e.g. PDF)
    /// report a single chapter.
    pub fn count_chapters(&self) -> Result<i32, Error> {
        unsafe { ffi_try!(mupdf_count_chapters(context(), self.inner)) }
    }

    /// Number of pages in a single chapter. Only this chapter is laid out, so
    /// counting page by page across a reflowable document never lays out the
    /// whole book up front.
    pub fn count_chapter_pages(&self, chapter: i32) -> Result<i32, Error> {
        unsafe { ffi_try!(mupdf_count_chapter_pages(context(), self.inner, chapter)) }
    }

    /// Load a page within a chapter. For reflowable documents this lays out only
    /// the chapter containing the requested page.
    pub fn load_chapter_page(&self, chapter: i32, page: i32) -> Result<Page, Error> {
        unsafe { ffi_try!(mupdf_load_chapter_page(context(), self.inner, chapter, page)) }
            // SAFETY: We're trusting the FFI layer here to provide a valid pointer
            .and_then(|fz_page| unsafe { Page::from_raw(fz_page) })
    }

    /// Map an absolute page number to a [`Location`]. For reflowable documents
    /// this lays out chapters up to (but not beyond) the one containing the page.
    pub fn location_from_page_number(&self, page_number: i32) -> Result<Location, Error> {
        unsafe { ffi_try!(mupdf_location_from_page_number(context(), self.inner, page_number)) }
            .map(|loc| Location {
                chapter: loc.chapter.max(0) as u32,
                page_in_chapter: loc.page.max(0) as u32,
                page_number: page_number.max(0) as u32,
            })
    }

    /// Map a [`Location`] back to an absolute page number.
    pub fn page_number_from_location(&self, chapter: i32, page: i32) -> Result<i32, Error> {
        unsafe {
